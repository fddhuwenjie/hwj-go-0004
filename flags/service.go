...
func scheduleActive(schedule *Schedule, at time.Time) (bool, error) {
	...
	loc, err := time.LoadLocation(strings.TrimSpace(schedule.Location))
	if err != nil {
		return false, fmt.Errorf("%w: location: %v", ErrInvalidInput, err)
	}
	start, err := time.ParseInLocation(localTimeLayout, schedule.Start, loc)
	if err != nil {
		return false, fmt.Errorf("%w: schedule start", ErrInvalidInput)
	}
	end, err := time.ParseInLocation(localTimeLayout, schedule.End, loc)
...
func ruleMatches(rule Rule, attributes map[string]string) bool {
	for _, condition := range rule.Conditions {
		actual, ok := attributes[condition.Attribute]
		if !ok {
			return false
		}
		matched := false
